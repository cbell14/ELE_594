function Conv1DNetwork_Testing(N_tv,N_test,folds,numClasses,miniBatchSize,SNR_list,testDIR,outputDIR)
%% Setup
data = [];
order = [];
SNRdB = [];
...
load([outputDIR,'TrainingData_Scaling_Terms.mat'])
for ii=1:1:size(data,1)
    X_Test{ii,1} = (2*(data{ii,1} - min_data) / (max_data - min_data)) - 1;
end
Y_Test = labels;

figure;plot(X_Test{1,1})
figure;histogram(labels)

%% Make Predictions & Visualize Performance
load([outputDIR,'1DConvNetwork.mat']);
preds = zeros(folds,size(data,1),numClasses);
Acc_peakSNR = NaN(folds,length(SNR_list));
totalSNR_vec = min(totalSNR):1:max(totalSNR);
Acc_totalSNR = NaN(folds,length(totalSNR_vec));

for kk=1:1:folds
    Y_Pred(kk,:,:) = predict(net(kk),X_Test, ...
